// Heavily inspired by https://github.com/openid/AppAuth-Android/blob/master/library/java/net/openid/appauth/AuthorizationManagementActivity.java
...
class CriiptoVerifyActivity : AppCompatActivity() {
  ...
  override fun onResume() {
    ...
    /**
     * Detected intermediate response/redirect from MitID app
     * kill this activity so we can resume to the primary activity with the browser
     */
    if (intermediateResponse != null) {
      Log.v(TAG, "CriiptoVerifyActivity.onResume: intermediate response" + getIntent().getData().toString())
      finish()
      return
    }
  ...
  companion object {
    fun createStartIntent(context: Context, params: StartParams) : Intent {
    ...
    private fun createBaseIntent(context: Context) : Intent {
      return Intent(context, CriiptoVerifyActivity::class.java).apply {
